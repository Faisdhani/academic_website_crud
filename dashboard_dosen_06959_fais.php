<?php
    ...
    $start = ($page > 1) ? ($page * $limit) - $limit : 0; // Hitung data awal

    // Query untuk menghitung total data
    $result_total = mysqli_query($koneksi, "SELECT COUNT(*) AS total FROM dosen");
    $total_data = mysqli_fetch_assoc($result_total)['total'];

    // Hitung total halaman
    $total_pages = ceil($total_data / $limit);

    // Query untuk data dosen dengan limit
    $result = mysqli_query($koneksi, "SELECT * FROM dosen LIMIT $start, $limit");
?>
...
<head>
    ...
    <style>
        ...
        .table td.namadosen {
            text-align: left; /* Left-align the 'Nama Dosen' column */
        }
        .action-links a {
            margin-right: 5px;
            font-size: 12px;
...
<div class="container">
    <h2 class="text-center my-4 custom-heading">Daftar Dosen</h2>
    <div class="text-center"><a class="btn btn-primary btn-lg shadow-lg rounded-pill text-white" href="printdosen_06959_fais .php"><i class="fas fa-print me-2"></i>Print</a></div>
    <div class="d-flex justify-content-between mb-4">
        <a href="tambah_dosen_06959_fais.php" class="btn btn-success"><i class="fas fa-plus-circle"></i> Tambah Dosen</a>
        <form class="form-inline" role="search">
            <input class="form-control mr-2" type="search" id="searchInput" placeholder="Cari dosen..." aria-label="Search">
        ...
        <table class="table table-hover">
            <thead class="thead-light">
                <tr>
                    ...
                    <th>NPP</th>
                    <th>Nama Dosen</th>
                    <th>Homebase</th> <!-- Centered Homebase -->
                    <th>Aksi</th>
                </tr>
            </thead>
            <tbody>
                <?php if (mysqli_num_rows($result) > 0): ?>
                    <?php while($row = mysqli_fetch_assoc($result)): ?>
                        <tr class="data-container">
                            <td><?= $row['id']; ?></td>
                            <td><?= $row['npp']; ?></td>
                            <td class="namadosen"><?= $row['namadosen']; ?></td> <!-- Left-aligned Nama Dosen -->
                            <td><?= $row['homebase']; ?></td>
                            <td class="action-links">
                                <a href="editdosen_06959_fais.php?id=<?= $row['id']; ?>" class="btn btn-warning btn-sm"><i class="fas fa-edit"></i> Edit</a>
                                <a href="hapusdosen_06959_fais.php?id=<?= $row['id']; ?>" class="btn btn-danger btn-sm" onclick="return confirm('Yakin ingin menghapus?');"><i class="fas fa-trash-alt"></i> Hapus</a>
                            </td>
                        </tr>
                    <?php endwhile; ?>
                <?php else: ?>
